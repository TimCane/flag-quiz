...
|-------|------|----------|-------------|
| `id` | string | Yes | UUID |
| `mode` | Mode enum | Yes | CLASSIC, PICK_THE_FLAG, or PICK_THE_COUNTRY |
| `exit_condition` | ExitCondition enum | Yes | NORMAL, STREAK, or SPEED |
| `quick` | number | Yes | Quick mode flag (0 or 1) |
...
Adds a `guess` (nullable string for skipped attempts) and `forgotten` (0 or 1) field to the base attempt fields.

### PickFlagAttemptSchema / PickCountryAttemptSchema

Adds `guess` (selected option code) and `options` (JSON string array of available options) fields.
...
| Enum | Values |
|------|--------|
| `Mode` | CLASSIC, PICK_THE_FLAG, PICK_THE_COUNTRY |
| `ExitCondition` | NORMAL, STREAK, SPEED |
